...
esp_err_t app_nvs_save_sta_creds(void) {
    ...
    if (wifi_sta_config) {
        ...
        if (esp_err != ESP_OK) {
            printf("app_nvs_save_sta_creds: Error (%s) commiting credentials to NVS!", esp_err_to_name(esp_err));
            return esp_err;
        }
...
esp_err_t app_nvs_load_sta_creds(void) {
    nvs_handle handle;
    esp_err_t esp_err;
    ESP_LOGI(TAG, "app_nvs_save_sta_creds: Loading Wifi credentials from flash");
    ...
    if (nvs_open(app_nvs_sta_creds_namespace, NVS_READONLY, &handle) == ESP_OK) {
        ...
        if (esp_err != ESP_OK) {
            free(wifi_config_buff);
            printf("app_nvs_load_sta_creds: Error (%s) retrieving Password!", esp_err_to_name(esp_err));
            return false;
        }
        ...
        return wifi_sta_config->sta.ssid[0] != '\0'; // If this is not equal to null character it means that the SSID is loaded
    } else {
        return false;
    }
}
...
esp_err_t app_nvs_clear_sta_creds(void) {
    ...
    if (esp_err != ESP_OK) {
        printf("app_nvs_clear_sta_creds: Error (%s) erasing station mode credentials!", esp_err_to_name(esp_err));
        return esp_err;
    }
