...
// ESP32 timer configuraiton passed to esp_timer_create.
...
/**
 * Checks the g_fw_update_status and create the fw_update_reset timer if the g_fw_update_status is true
*/
static void http_server_fw_update_reset_time(void) {
    if (g_fw_update_status == OTA_UPDATE_SUCCESSFUL) {
        ESP_LOGI(TAG, "http_server_OTA_update_handler: FW update successful starting FW update reset timer.");

        // Give the web page a chance to receive an acknowledgement back and initialize the timer
        ESP_ERROR_CHECK(esp_timer_create(&fw_update_reset_args, &fw_update_reset));
        ESP_ERROR_CHECK(esp_timer_start_once(fw_update_reset, 8000000));
    } else {
        ESP_LOGI(TAG, "http_server_OTA_update_handler: FW update unsuccessful");
    }
}
        ...
        if (xQueueReceive(http_server_monitor_queue_handle, &msg, portMAX_DELAY)) {
            switch (msg.msgID) {
                ...
                case HTTP_MSG_OTA_UPDATE_SUCCESSFUL:
                    ESP_LOGI(TAG, "HTTP_MSG_OTA_CONNECT_SUCCESSFUL");
                    g_fw_update_status = OTA_UPDATE_SUCCESSFUL;
                    http_server_fw_update_reset_time();
...
/**
 * JQuery get handler request when accessing the web page.
 * @param req HTTP request for which the uri need to the handled
 * @return ESP_OK
*/
...
static esp_err_t http_server_ota_update_handler(httpd_req_t *req) {
    ...
    do {
        ...
        // If this is the first date we are receiving
        // If so, it will have the inforamtion in the header that we need.
        if (!is_req_body_started) {
            ...
            printf("http_server_OTA_update_handler: ORA file size %d\r\n", content_length);
            ...
            if (err != ESP_OK) {
                ...
                ESP_LOGI(TAG, "http_server_OTA_update_handler: Writing to partition subtype %d at offset 0x%" PRIu32 "\r\n", update_partition->subtype, update_partition->address);
            }

            //Write this first part of the data
            esp_ota_write(ota_handle, body_start_p, body_part_len);
            content_received += body_part_len; // added based on video comment
        } else {
            // Write OTA data
    ...
    if (esp_ota_end(ota_handle) == ESP_OK) {
        ...
        if (esp_ota_set_boot_partition(update_partition) == ESP_OK) {
            ...
            ESP_LOGI(TAG, "http_server_OTA_update_handler: Next boot partition subtype %d at offset 0x%" PRIu32 " \r\n", boot_partition->subtype, boot_partition->address);
    ...
    // We won't update the global variable throughout the file, so send the message about the status
...
/**
 * WifiConnect.json handler is inoked after the connect button is pressed
...
static esp_err_t http_server_wifi_connect_json_handler(httpd_req_t *req) {
    ...
    // Update Wifi networks configuration and let the wifi application know
...
/**
 * WifiConnectInfo.json handler updates the web page with the wifi information
 * @param req HTTP request for which the uri needs to be handled.
 * @return ESP_OK
*/
static esp_err_t http_server_get_wifi_connect_info_json_handler(httpd_req_t *req) {
    ...
    if (g_wifi_connect_status == HTTP_WIFI_STATUS_CONNECT_SUCCESS) {
        ...
        esp_ip4addr_ntoa(&ip_info.ip, ip, IP4ADDR_STRLEN_MAX); // ntoa = network to ascii
 ...
 * @return ESP_OK
*/
static esp_err_t http_server_wifi_disconnect_info_json_handler(httpd_req_t *req) {
    ESP_LOGI(TAG, "/WifiDisconnectInfo.json requested");
...
static httpd_handle_t http_server_configure(void) {
    ...
    // Adujst the default priority to 1 less than the wifi application
    ...
    if (httpd_start(&http_server_handle, &config) == ESP_OK) {
        ...
        httpd_uri_t wifi_disconnect_info_json = {
                .uri = "/wifiDisconnect.json",
                .method = HTTP_DELETE,
                .handler = http_server_wifi_disconnect_info_json_handler,
                .user_ctx = NULL
        };
        httpd_register_uri_handler(http_server_handle, &wifi_disconnect_info_json);
...
void http_server_stop(void) {
    if (http_server_handle) {
        ...
        ESP_LOGI(TAG, "http_server stop: stopping HTTP server");
        http_server_handle = NULL;
    }
    if (task_http_server_monitor) {
        vTaskDelete(task_http_server_monitor);
        ESP_LOGI(TAG, "http_server stop: stopping HTTP server monitor");
...
void http_server_fw_update_reset_callback(void *arg) {
    ESP_LOGI(TAG, "http_server_OTA_update_handler: Timer timed-out, restarting the device");
    esp_restart();
}