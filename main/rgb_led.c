...
// handle for rgb_led_pwd_init
bool g_pwm_init_handle = false;

/**
 *  Initiaizes the RGB LED settings per channel, including
...
static void rgb_led_set_color(uint8_t red, uint8_t green, uint8_t blue) {
    // Value should be 0 to 255 for a 8 bit color
...
void rgb_led_wifi_app_started(void) {
    ...
    rgb_led_set_color(0, 255, 255); // Red (Opposite RGB 255 0 0)
}
...
void rgb_led_http_server_started(void){
    ...
    rgb_led_set_color(0, 127, 255); // Orange (Opposite RGB 255 128 0)
}
...
void rgb_led_wifi_connected(void) {
    ...
    rgb_led_set_color(255, 0, 255); // Green (Opposite RGB 0 255 0)
}
