 ...
 * @param event_base the base id of he event to register for
...
/**
 * Initializes the TCP stat and default Wifi configuration
*/
...
static void wifi_app_soft_ap_config(void) {
    ...
    inet_pton(AF_INET, WIFI_AP_IP, &ap_ip_info.ip); // > Assign access point's static IP, GW, netmask
    ...
    ESP_ERROR_CHECK(esp_netif_dhcps_start(esp_netif_ap)); // > start the AP DHCP server (for connecting station e.g. your mobile device)

    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_APSTA)); // > Setting the mode as Access Point / Station Mode
    ESP_ERROR_CHECK(esp_wifi_set_config(ESP_IF_WIFI_AP, &ap_config)); // > Set our confiugration
    ESP_ERROR_CHECK(esp_wifi_set_bandwidth(ESP_IF_WIFI_AP, WIFI_AP_BANDWIDTH)); // > Our default bandwidth 20 MHz
    ESP_ERROR_CHECK(esp_wifi_set_ps(WIFI_AP_POWER_SAVE)); // > PowerSave set to "NONE"
}
        ...
        if (xQueueReceive(wifi_app_queue_handle, &msg, portMAX_DELAY)) {
            ...
            case WIFI_APP_MSG_LOAD_SAVED_CREDENTIALS:
                ...
                if (app_nvs_load_sta_creds()) {
                    ...
                    ESP_LOGI(TAG, "Unable to loaded station configuration");
                }
            ...
            case WIFI_APP_MSG_CONNECTING_FROM_HTTP_SERVER:
                ...
                // Set current number of retry to zero
                g_retry_number = 0;
            ...
            case WIFI_APP_MSG_USER_REQUESTED_STA_DISCONNECT:
                ...
                if (eventBits & WIFI_APP_STA_CONNECTED_GOT_IP_BIT) {
                    ...
                    rgb_led_http_server_started(); // > TODO: rename the status LED to a name more meaningful
                }
            ...
            case WIFI_APP_MSG_STA_DISCONNECTED:
                ...
                if (eventBits & WIFI_APP_CONNECTING_USING_SAVED_CREDS_BIT) {
                    ...
                    app_nvs_clear_sta_creds();
                } else if (eventBits & WIFI_APP_MSG_CONNECTING_FROM_HTTP_SERVER) {
                    ...
                    ESP_LOGI(TAG,"WIFI_APP_MSG_STA_DISCONNECTED: ATTEMPT FAILED, CHECK WIFI POINT AVAILABILITY");
                    // Adjust this case to your need - maybe you want to keep trying to connect...
                }
